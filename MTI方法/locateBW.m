...
tStart=tic;
%% 读取数据
path='C:\Users\24202\Desktop\folder\workspace\matlab\prj\passiveRadar\data\20231110\';

% 监测信道
file_surv='main_move_10m_01';
filename_surv=strcat(path,file_surv);
sig_surv=read_complex_binary(filename_surv);
sig_surv=upsample(sig_surv,2);

% 参考信道
file_ref='ref_move_10m_01';
...
backNum = 0;
for i = 1:Num_slow
    sig_ref_pc(:,i) = pulseCompressionFunc(sig_ref(:,i),sig_ref(:,i));
...
backNum = 0;
for i = 1:Num_slow
    sig_surv_pc(:,i) = pulseCompressionFunc(sig_surv(:,i),sig_ref(:,i));
...
subplot(1,2,1)
x = 1:Num_slow;
y = min(maxindex_sig_ref_pc):max(maxindex_sig_ref_pc)+500;
z = abs(sig_ref_pc(y,:));
imagesc(x,y,z)
xlabel('slow time')
ylabel('fast time')
...
subplot(1,2,2)
x = 1:Num_slow;
y = min(maxindex_sig_surv_pc):max(maxindex_sig_surv_pc)+500;
z = abs(sig_surv_pc(y,:));
imagesc(x,y,z)
xlabel('slow time')
ylabel('fast time')
...
backNum = 0;
for i = 1:Num_slow
    sig_ref_aline_phase_pc(:,i) = pulseCompressionFunc(sig_ref_aline_phase(:,i),sig_ref(:,i));
...
backNum = 0;
for i = 1:Num_slow
    sig_surv_aline_phase_pc(:,i) = pulseCompressionFunc(sig_surv_aline_phase(:,i),sig_ref(:,i));
...
backNum = 0;
for i = 1:Num_slow
    sig_surv_aline_phase_mti_pc(:,i) = pulseCompressionFunc(sig_surv_aline_phase_mti(:,i),sig_ref(:,i));
